...
def test_interactive_rerun_prompts_default_to_previous_values(tmp_path):
    ...
    with patch(
        ...
        # recyclarr confirm, homepage confirm, dashy confirm,
        # audiobookshelf confirm, PUID, PGID, timezone all hit enter to
        # accept their (previous-state-derived) defaults; the generate
        # confirm has no default so needs an explicit "y", then decline
        # the final start confirm with "n".
...
def test_non_interactive_light_with_explicit_dashy_flag(tmp_path):
    ...
    with patch(
        ...
        result = runner.invoke(
            ...
            [
                "--tier", "light", "--media-path", media_path,
                "--non-interactive", "--yes", "--dashy", "--no-homepage", "--no-vpn"
            ]
        )
    ...
    config = mock_write_stack.call_args[0][0]
    assert config.enabled_optional == {"dashy"}


def test_non_interactive_fresh_install_defaults_dashy_disabled(tmp_path):
    """
    Unlike Homepage, Dashy is opt-in on a fresh install - it's an
    alternative for users who prefer its UI, not a broadly-valuable
    default the way Homepage was promoted to.
    """
...
def test_non_interactive_light_with_explicit_audiobookshelf_flag(tmp_path):
    ...
    with patch(
        ...
        result = runner.invoke(
            ...
            [
                "--tier", "light", "--media-path", media_path,
                "--non-interactive", "--yes", "--audiobookshelf",
                "--no-homepage", "--no-dashy", "--no-vpn"
            ]
        )
    ...
    config = mock_write_stack.call_args[0][0]
    assert config.enabled_optional == {"audiobookshelf"}


def test_non_interactive_fresh_install_defaults_audiobookshelf_disabled(tmp_path):
    ...
    config = mock_write_stack.call_args[0][0]
    assert "audiobookshelf" not in config.enabled_optional
