...
- [ ] **Cloudflare Tunnel support** — real gap named while writing the Cloudflare DNS walkthrough (2026-08-18): today, Traefik+domain+`--cloudflare-dns` gets real Let's Encrypt certs via DNS-01, but reaching the host at all still needs ports 80/443 forwarded from the router — Cloudflare's proxy hides the origin IP from public DNS lookups, it doesn't remove the need for a real inbound path. A `cloudflared` sidecar (custom-mode, alongside Traefik) would remove that requirement entirely — no forwarded ports, outbound-only connection to Cloudflare's edge. Needs design: a scoped Tunnel token (separate credential from `CF_DNS_API_TOKEN`, created in the Zero Trust dashboard, not the same permissions), whether it replaces or layers with the existing Traefik entrypoint, and real verification against a live tunnel before calling it shipped, same bar as every other integration here.
